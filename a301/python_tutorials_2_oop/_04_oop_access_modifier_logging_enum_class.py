...
logging.basicConfig(filename="araba_log.txt", level=logging.INFO, format='%(name)s: %(levelname)s: %(lineno)s = %(funcName)s ,  %(message)s , %(asctime)s ')
...
class Araba:
    ...
    def __init__(self,
        ...
        logging.info("Araba sınıfına yeni bir araba oluşturuldu: Marka: {self.__marka}, Modeling: {self._modeling}, Yıl: {self.yil}, Renk: {self.renk}")
        print(f"{Renk.MAVI.value} Araba sınıfından yeni bir örnek oluşturuldu")

    # METHOD
    def goster_bilgileri(self) -> None:
        print(f"{Renk.SARI.value} Marka: {self.__marka}, modeling: {self._modeling}, Yıl: {self.yil}, Renk: {self.renk}")
    ...
    def set_marka(self,marka:str) -> None:
        ...
        self.__marka = marka
        logging.info(f"{Renk.MAVI.value} Araba markası güncellendi: Marka: {self.__marka}")
    ...
    def set_modeling(self, modeling:str) -> None:
        ...
        logging.info(f"{Renk.MAVI.value} Araba modelini güncellendi: Modeling: {self._modeling}")
    ...
    def set_yil(self, yil:str) -> None:
        ...
        try:
            ...
            print(f"{Renk.KIRMIZI.value} Geçersiz Tarih Formatında yazdınız. Lütfen YYYY-MM-DD şeklinde yazınız")
    ...
    def set_renk(self, renk:str) -> None:
        ...
        self.renk = renk
        logging.info(f" {Renk.MAVI.value} Araba rengini güncellendi: Renk: {self.renk}")
